...
*** Variables ***
&{FIREFOX_AUTOMATION}    browser=firefox    url=https://www.way2automation.com/angularjs-protractor/webtables/    title=Protractor practice website - WebTables
...
&{USER_JANE}    first_name=Jane    last_name=Smith    user_name=jane    password=12345    customer=15    role=Admin    email=jane@example.com    cell_phone=+1 9999 00000    str_customer=Company AAA

*** Test Cases ***
Add And Validate User
    [Documentation]    Add user Harry and validate the data has been added in the first row
    [Setup]    Open Browser To URL    &{FIREFOX_AUTOMATION}
    Add User
...
Open Browser To URL
    ...
    Call Method    ${options}    set_preference    browser.startup.homepage    https://www.way2automation.com/angularjs-protractor/webtables/
    Open Browser    ${ARG1['url']}    ${ARG1['browser']}    options=${options}
